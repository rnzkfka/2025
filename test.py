...
# 3-1. 대학교 홈페이지
# -------------------------
university_urls = {univ: f"https://www.{univ.replace('(', '').replace(')', '').replace(' ', '').replace('대학교','')}.ac.kr" for univ_list in universities.values() for univ in univ_list}
...
for city_univs in universities.values():
    for univ in city_univs:
        departments[univ] = ["컴퓨터공학과", "경영학과", "간호학과", "전기공학과", "디자인학과"]  # 예시 기본 학과
